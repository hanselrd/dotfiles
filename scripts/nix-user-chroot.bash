...
file_chmod__48_v0() {
    ...
    echo "The file ${path_36} doesn't exist"'!'""
    ret_file_chmod48_v0=''
    return 1
}
...
file_download__241_v0() {
    ...
    if [ "${ret_is_command125_v0__15_9}" != 0 ]; then
        curl -L -o "${path_30}" "${url_29}" >/dev/null 2>&1
        __status=$?
    elif [ "${ret_is_command125_v0__18_9}" != 0 ]; then
        wget "${url_29}" -P "${path_30}" >/dev/null 2>&1
        __status=$?
    elif [ "${ret_is_command125_v0__21_9}" != 0 ]; then
        aria2c "${url_29}" -d "${path_30}" >/dev/null 2>&1
        __status=$?
    else
...
# nix_user_chroot_download(version: Text, arch: Text, path: Text)
nix_user_chroot_download__244_v0() {
    local version_26="${1}"
    local arch_27="${2}"
    local path_28="${3}"
    file_download__241_v0 "https://github.com/nix-community/nix-user-chroot/releases/download/${version_26}/nix-user-chroot-bin-${version_26}-${arch_27}-unknown-linux-musl" "${path_28}"
    __status=$?
    if [ "${__status}" != 0 ]; then
        ret_nix_user_chroot_download244_v0=''
        return "${__status}"
    fi
    ret_nix_user_chroot_download244_v0=''
    return 0
...
ret_env_var_get121_v0__10_27="${ret_env_var_get121_v0}"
install_dir_5="${ret_env_var_get121_v0__10_27}/.bootstrap"
nix_user_chroot_bin_6="${install_dir_5}/nix-user-chroot"
dir_create__45_v0 "${install_dir_5}"
__status=$?
if [ "${__status}" != 0 ]; then
    exit "${__status}"
fi
nix_user_chroot_download__244_v0 "1.2.2" "x86_64" "${nix_user_chroot_bin_6}"
__status=$?
if [ "${__status}" != 0 ]; then
    exit "${__status}"
fi
file_chmod__48_v0 "${nix_user_chroot_bin_6}" "755"
__status=$?
if [ "${__status}" != 0 ]; then
    exit "${__status}"
fi
