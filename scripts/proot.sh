...
ret_env_var_get98_v0__6_27="${ret_env_var_get98_v0}"
install_dir_3="${ret_env_var_get98_v0__6_27}/.bootstrap"
proot_bin_4="${install_dir_3}/proot"
dir_create__42_v0 "${install_dir_3}"
__status=$?
if [ "${__status}" != 0 ]; then
  exit "${__status}"
fi
file_download__146_v0 "https://proot.gitlab.io/proot/bin/proot" "${proot_bin_4}"
__status=$?
if [ "${__status}" != 0 ]; then
  exit "${__status}"
fi
file_chmod__45_v0 "${proot_bin_4}" "755"
__status=$?
if [ "${__status}" != 0 ]; then
  exit "${__status}"
fi
