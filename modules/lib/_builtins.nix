...
in
{
  getRandomString = length: builtin "random-string" [ (builtins.toString length) ];

  getDevicePartition = path: builtin "device-partition" [ path ];

  getDevice = path: builtin "device" [ path ];
